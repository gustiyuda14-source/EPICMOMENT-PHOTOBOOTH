export const HOW_TO_BOOK = [
  "Hubungi admin kami di WhatsApp untuk konsultasi kebutuhan acara Anda.",
  "Lakukan DP (down payment) sebesar Rp500.000, pelunasan H-3 acara (kami bantu ingatkan).",
  "Setelah DP, kami akan membantu Custom Layout Design (Frame Photo + GIF).",
  "Reschedule dapat dilakukan H-7 acara apabila masih ada slot kosong.",
  "Jika terjadi pembatalan, DP (down payment) hangus / tidak ada refund.",
  "Free akomodasi dalam kota Kendari — luar kota Kendari akomodasi ditanggung client.",
];

export const PAYMENT_NOTE =
  "Jangan melakukan transaksi selain dengan rekening resmi di atas.";

export const ON_SITE_REQUIREMENTS = [
  "Kami hadir 1 jam sebelum waktu operasional yang ditentukan client untuk set up alat.",
  "Lokasi photo booth tidak boleh terpapar matahari langsung / hujan, karena dapat merusak peralatan.",
  "Space booth 3x3 meter, dengan background yang disiapkan client atau simple background dari kami.",
  "Dibutuhkan sumber listrik terdekat — kami menyediakan kabel roll 10–15 meter untuk menjangkau sumber listrik.",
  "2 crew profesional standby untuk operasional photo booth dari persiapan sampai selesai acara.",
  "Untuk acara wedding, pengantin berkenan melakukan sesi foto di awal acara.",
];

export const WHY_PHOTOBOOTH = [
  {
    title: "Bentuk Apresiasi Kepada Tamu",
    description:
      "Photo booth adalah cara sederhana menunjukkan apresiasi kepada tamu yang hadir di acara Anda.",
  },
  {
    title: "Souvenir Anti Mainstream",
    description:
      "Cetakan foto instan jadi kenang-kenangan yang lebih berkesan dibanding souvenir biasa.",
  },
  {
    title: "Experience & Suasana Berbeda",
    description:
      "Menghadirkan pengalaman dan suasana seru yang tidak ditemukan di photo booth pada umumnya.",
  },
];

export const PRINT_COMPARISON = [
  { aspect: "Batasan utama", limited: "Jumlah lembar (kuota)", unlimited: "Durasi waktu sewa" },
  { aspect: "Berhenti cetak saat", limited: "Kuota habis", unlimited: "Waktu sewa habis" },
  { aspect: "Harga", limited: "Lebih murah", unlimited: "Lebih mahal" },
  {
    aspect: "Cocok untuk",
    limited: "Acara kecil / budget terbatas",
    unlimited: "Acara besar / tamu banyak",
  },
  {
    aspect: "Risiko biaya kertas",
    limited: "Ditanggung klien (terbatas)",
    unlimited: "Ditanggung vendor",
  },
];

export const EQUIPMENT_SPECS = [
  "Kamera DSLR Canon 24MP",
  "Layar sentuh 24 inci",
  "QR code untuk online share instan",
  "Printer DNP RX1HS — hasil cetak premium",
  "Studio lighting (softbox + flash) untuk hasil foto konsisten",
];
